...
class DDS(_algorithm):
    ...
    def sample(self, repetitions, trials=1, s_initial=[]):
        ...
        if len(s_initial) == 0:
            ...
            if not (np.all(s_initial <= self.best_value.parameters.maxbound) and np.all(s_initial >= self.best_value.parameters.minbound)):
        ...
        for trial in range(trials):
            ...
            repetions_left, trial_best_value = self.calc_initial_para_configuration(initial_iterations, trial, repetitions, s_initial)
