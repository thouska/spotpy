...
class dds(_algorithm):
    ...
    def sample(self, repetitions, trials=1, x_initial=np.array([])):
        ...
        self.logger.info('Starting the DDS algotrithm with '+str(repetitions)+ ' repetitions...')
        ...
        for trial in range(trials):
            ...
            self.logger.debug('Best solution found has obj function value of ' + str(objectivefunction_max) + ' at '
                  + str(repitionno_best) + '\n\n')
    ...
    def calc_initial_para_configuration(self, initial_iterations, trial, repetitions, x_initial):
        ...
        if initial_iterations > 1:
            self.logger.debug('Finding best starting point for trial ' + str(trial + 1) + ' using ' + str(
